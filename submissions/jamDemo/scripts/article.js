(function(module) {
  ...
  Article.createTable = function(callback) {
    webDB.execute(
      'CREATE TABLE articles (id INTEGER PRIMARY KEY, title VARCHAR(50), author VARCHAR(50), authorUrl VARCHAR(50), category TEXT, publishedOn VARCHAR, body VARCHAR)', // what SQL command do we run here inside these quotes?
      function(result) {
        console.log('Successfully set up the articles table.', result); //result should be null
      ...
      [
        {
          ...
          'sql': 'UPDATE articles SET author = Kevin Bacon WHERE author = Keven Barcon;', //fill out like we would, but it won't sun so run in console
      ...
      [
        {
          ...
          /* Note: this is an advanced admin option, so you will need to test
              out an individual query in the SQL console */
          'sql': 'DELETE FROM articles WHERE id = 10;',//same as above won't run
          'data': [this.id]
        }
      ],
      callback
